...
export default function ProfilePage() {
  ...
  const [formData, setFormData] = useState({
    ...
    bio: user?.user_metadata?.bio || "Passionate about technology and always looking to upgrade. I take great care of my items and provide detailed descriptions.",
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Disable editing state immediately to prevent double submissions
      setIsEditing(false)
      ...
      if (error) {
        ...
        // Re-enable editing if there's an error
        setIsEditing(true)
        toast({
          title: "Update failed",
  ...
  const handleCancel = () => {
    // Reset form data to original values from user metadata
    if (user) {
      setFormData({
        name: user.user_metadata?.name || user.email || "",
        email: user.email || "",
  ...
  const userListings = [] // TODO: Implement with Supabase products
                ...
                <div className="text-center mb-6">
                  <div className="relative inline-block">
                    <Avatar className="h-24 w-24 mx-auto mb-4">
                      <AvatarImage src={formData.avatar || "/placeholder.svg"} alt={user?.user_metadata?.name || user?.email} />
                      <AvatarFallback className="text-2xl">{(user?.user_metadata?.name || user?.email)?.charAt(0) || "U"}</AvatarFallback>
                    </Avatar>
                    <Button
                      size="sm"
                      variant="secondary"
                  ...
                  <h2 className="text-xl font-bold mb-2">{user?.user_metadata?.name || user?.email}</h2>
                ...
                <form onSubmit={handleSave} className="space-y-6">
                  ...
                  {isEditing && (
                    <div className="flex justify-end space-x-2">
                      ...
                      <Button type="button" onClick={handleCancel} variant="outline" size="sm" className="bg-transparent">
                        Cancel
                      </Button>
                    </div>
                  )}
